...
var sqlQuery = `SELECT * FROM students WHERE login = ? AND password = ?;select * from blogs`;
...
router.get('/home/:id', function (req, res, next) {
  ...
  var sqlQuery = `SELECT * FROM students WHERE user_id='${id}';select * from blogs;`;

  db.query(sqlQuery, function (err, results, fields) {  
    console.log(results)    
    const stud=results[0]
    const blogs=results[1]
    res.render('./student/home',{stud});
  });
  });
...
router.get('/blogs/:id', function (req, res, next) {
    const id =req.params.id
    var sqlQuery = `SELECT * FROM blogs`;
  
    db.query(sqlQuery, function (err, results, fields) {
    const blogs = results
    res.render('./student/blog.ejs',{blogs,id});
  });
  });
